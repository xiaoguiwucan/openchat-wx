...
## [openchat-wx 1.1.0] - 2026/08/02

### 新功能

- 新增可复用的多模型渠道，统一配置对话、识图、绘图和群聊总结模型。
- 新增模型渠道管理页面、连接测试和按全局/群聊/好友切换接口。
- 旧版 AI 配置会无损迁移为渠道，保留原字段兼容已有部署。

### BUG 修复

- 新增长连接 watchdog，修复协议长连接 EOF 后进程存活但无法接收新消息的问题。
- 群聊选定渠道后不再被旧版残留 Base URL 或模型字段覆盖。
- 长期记忆提取遵循会话渠道选择；第三方渠道不支持流式或工具调用时自动降级。
