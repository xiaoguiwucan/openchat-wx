...
type ChatRoomSettings struct {
	...
	ChatRoomID                 string               `gorm:"column:chat_room_id;type:varchar(64);default:'';uniqueIndex:idx_chat_room_id;comment:群聊微信ID" json:"chat_room_id"`
	AIProviderID               *int64               `gorm:"column:ai_provider_id;index;comment:群聊模型渠道ID，为空时使用旧配置或全局渠道" json:"ai_provider_id"`
	ChatAIProviderID           *int64               `gorm:"column:chat_ai_provider_id;index;comment:群聊AI回复渠道ID，为空时继承全局" json:"chat_ai_provider_id"`
	ImageRecognitionProviderID *int64               `gorm:"column:image_recognition_provider_id;index;comment:群聊图像识别渠道ID，为空时继承全局" json:"image_recognition_provider_id"`
	ImageGenerationProviderID  *int64               `gorm:"column:image_generation_provider_id;index;comment:群聊图像生成渠道ID，为空时继承全局" json:"image_generation_provider_id"`
	SummaryAIProviderID        *int64               `gorm:"column:summary_ai_provider_id;index;comment:群聊总结渠道ID，为空时继承全局" json:"summary_ai_provider_id"`
	...
	FreeReplyLevel             *string              `gorm:"column:free_reply_level;type:varchar(20);comment:自由回复档位" json:"free_reply_level"`
	FreeReplyCooldownSeconds   *int                 `gorm:"column:free_reply_cooldown_seconds;comment:自由回复冷却秒数" json:"free_reply_cooldown_seconds"`
	FreeReplyDailyLimit        *int                 `gorm:"column:free_reply_daily_limit;comment:每群每日自由回复上限" json:"free_reply_daily_limit"`
	...
	ImageGenerationModel       *string              `gorm:"column:image_generation_model;type:varchar(100);default:'';comment:图像生成使用的模型名称" json:"image_generation_model"`
